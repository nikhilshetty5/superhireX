...
const App: React.FC = () => {
      ...
      <div className="h-screen bg-black overflow-hidden relative flex flex-col">
            ...
            {stack.length > 0 ? (
              stack.map((item) => (
                <SwipeCard 
                  ...
                  rightLabel={role === UserRole.SEEKER ? "APPLY" : "HIRE"}
                  leftLabel="SKIP"
                >
              ...
              >
                <div className="w-20 h-20 bg-white/5 rounded-full flex items-center justify-center mb-6 border border-white/10">
                  <RefreshCcw size={32} className="text-white/40 animate-spin" />
                </div>
        ...
        <footer className="p-8 flex items-center justify-center gap-6 z-50">
          <button 
            ...
            onClick={() => stack.length > 0 && handleSwipe('left', stack[stack.length - 1].id)}
            className="w-16 h-16 rounded-full bg-red-500/10 border border-red-500/20 flex items-center justify-center text-red-500 hover:bg-red-500/20 transition-all active:scale-90 disabled:opacity-30"
          >
            <X size={32} />
          </button>
          <button 
             disabled={stack.length === 0}
             onClick={() => stack.length > 0 && handleSwipe('right', stack[stack.length - 1].id)}
             className="w-20 h-20 rounded-full bg-green-500/10 border border-green-500/20 flex items-center justify-center text-green-500 hover:bg-green-500/20 shadow-lg shadow-green-500/10 transition-all active:scale-90 disabled:opacity-30"
          >
  ...
  }, [role, isAuthenticated, user, jobs, candidates, handleSwipe]);
